	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				preg_match('/^<a.*?href=(["\'])(.*?)\1.*$/', $item, $m);
				$itemUrl = isset($m[2]) ? $m[2] : '';
				...
				$itemN = $itemUrl ? sprintf( '<meta itemprop="item" content="%s" />', esc_url( $itemUrl ) ) : '';


				// Build the list item.
				$breadcrumb .= sprintf( '<%1$s %2$s>%3$s%4$s%5$s</%1$s>', tag_escape( $this->args['item_tag'] ),$attributes, $item, $meta, $itemN );
			}
